...
//     } else {
//         bpf_printk("⚠️ Failed to capture syscall path: PID=%d, ret=%ld", pid, ret);
...
//         bpf_printk("⚠️ No syscall correlation for PID=%d", pid);
//     }
...
//     } else {
//         bpf_printk("⚠️ Failed syscall capture: PID=%d, ret=%ld", pid, ret);
//     }
...
int BPF_PROG(content_aware_toctou_detector, struct linux_binprm *bprm)
{
    ...
    if (ret <= 0) {
        ...
        bpf_printk("⚠️ LSM path fallback: PID=%d, Comm=%s", pid, actual_path);
    ...
    if (stored_context && stored_context->has_syscall_path) {
        ...
        event->uid = bpf_get_current_uid_gid() & 0xFFFFFFFF;
        bpf_printk("⚠️ No syscall correlation for PID=%d, actual=%s", pid, actual_path);
    }
